# adj.awk -- adjust lines of text per options 
# 
...
$0 ~ startblank { 
  if ( outline != "" ) { 
    ...
    else outline = outline " " word[i] # 1 space 
    }
  } 
} 
...
$0 ~ startwords { 
  ...
  if ( testlen > linelen ) { 
    ...
    if ( outline == "" ) outline = word[i] 
    else { 
      if ( match( ".!?:;", "\\" substr( outline, length( outline ), 1 )) ) outline = outline " " word[i] # 2 spaces 
    else 
